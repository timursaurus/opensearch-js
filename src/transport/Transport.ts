...
import { gzip, unzip, createGzip } from 'zlib';
...
export class Transport {
  ...
  sniff(opts: TransportSniffOptions, callback: (...args: any[]) => void = NOOP): void {
...
export function randomSelector(connections) {
...
export function generateRequestId() {
  ...
  let nextRequestId = 0;
  return () => (nextRequestId = (nextRequestId + 1) & MAX_INT);
}

export function lowerCaseHeaders(oldHeaders: Record<string, string>) {
...
export function validateMemoryPercentage(percentage: number): number {
